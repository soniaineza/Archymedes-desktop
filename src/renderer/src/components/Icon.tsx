...
const ICONS = {
  logo: <path d="M12 12c0-.6.5-1 1-1s1.2.6 1.2 1.4c0 1.2-1 2-2.2 2s-2.6-1.2-2.6-2.8 1.6-3.4 3.6-3.4 4.2 1.8 4.2 4.4-2.2 5-5.2 5-5.8-2.4-5.8-6 2.8-6.6 6.6-6.6 7.4 3 7.4 7.4" />,
  ...
  diff: <path d="M12 3v10M7 8h10M7 19h10" />,
  external: (
    <>
  ...
  eyeOff: (
    <>
      <path d="M9.9 4.24A9 9 0 0 1 12 4c6.5 0 10 8 10 8a18 18 0 0 1-2.16 3.19" />
...
type Props = Omit<SVGProps<SVGSVGElement>, "name"> & {
  name: IconName;
  size?: number;
  /** Mirror in right-to-left layouts (directional glyphs like chevrons and arrows). */
  flipRtl?: boolean;
};

export function Icon({ name, size = 16, flipRtl, className, ...rest }: Props) {
  const classes = ["icon", flipRtl ? "icon-flip-rtl" : "", className ?? ""].filter(Boolean).join(" ");
  return (
    <svg
      width={size}
      height={size}
